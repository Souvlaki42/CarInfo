...
const { Translator, ensureAuthenticated, ensureNotAuthenticated, emailSend, generateCrypto } = require("../config/api");

const router = express.Router();

router.get("/", ensureAuthenticated, async (req, res) => {
	const cars = await Cars.find().sort({ createdAt: "desc" });
	res.render("index", { cars: cars, user: req.user.username });
});
...
router.get("/verify/:token", ensureNotAuthenticated, async (req, res) => {
...
router.post("/settings", (req, res) => {
	...
	res.json([{item: section, type: typeof section}, {item: user, type: typeof user}]);
});
...
router.post("/", ensureAuthenticated, async (req, res) => {
	...
	if (errors.length > 0) {
		...
		res.render("index", { cars: cars, user: req.user.username, search: req.body.query, errors: errors });
	} else {
		if (select == "1") {
		...
		res.render("index", { cars: cars, user: req.user.username, search: req.body.query });
	}
});
